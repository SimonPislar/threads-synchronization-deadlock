...
void spin_lock(volatile int *target) {
    /* sTODO: Implement the lock operation for a test-and-set spinlock. */
    while(__sync_lock_test_and_set(target, 1)) {
    }
}
...
inc_atomic(void *arg __attribute__((unused)))
{
    ...
    for (i = 0; i < INC_ITERATIONS; i++) {
        ...
        __sync_fetch_and_add(&counter, INCREMENT); // You need to replace this.
    }
...
dec_atomic(void *arg __attribute__((unused)))
{
    ...
    for (i = 0; i < DEC_ITERATIONS; i++) {
        ...
        __sync_fetch_and_sub(&counter, DECREMENT); // You need to replace this.
    }
